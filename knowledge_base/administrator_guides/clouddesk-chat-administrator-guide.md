...
*This guide covers administrative configuration and operation of CloudDesk Chat. It is intended for account administrators and IT/security staff responsible for managing the platform. For end-user instructions, refer to the CloudDesk Chat User Manual. For pricing and plan details, refer to the Corvex Cloud Pricing Guide.*
...
The Admin Dashboard is the central console for configuring and overseeing your organization's CloudDesk Chat deployment. It is accessible only to users with the Administrator role and is separate from the standard agent workspace described in the CloudDesk Chat User Manual.

### 1.1 Accessing the Admin Dashboard

1. Log in to CloudDesk Chat with an account holding the Administrator role.
...
- **Widget & Channels** — configure the chat widget, routing, and connected channels
...
- **Integrations** — manage connected CRM, e-commerce, collaboration, and API integrations
...
The top of the Admin Dashboard displays a summary panel showing current seat usage against your plan's limits, storage usage, and any outstanding security recommendations Corvex has flagged for your account (for example, an unused API key or an agent without SSO enrollment, where applicable).
...
4. Click **Send Invitation**. The user will receive an email invitation as described in the CloudDesk Chat User Manual.
...
2. Deactivated users lose login access immediately but their historical activity (conversations handled, notes added) remains intact in the account's records.
...
CloudDesk Chat uses a role-based structure that is consistent across the broader Corvex Cloud platform, ensuring a user's permissions behave the same way whether they're working in CloudDesk Chat, CloudDesk Tickets, or CloudDesk Analytics.

### 3.1 Standard Roles

- **Agent** — handles assigned and queued conversations; no administrative or configuration access
- **Team Lead / Supervisor** — includes all Agent capabilities, plus visibility into team-wide queues, the ability to reassign conversations, and access to team-level reporting
- **Administrator** — full configuration access, including widget setup, routing, integrations, security settings, and user management
- **Read-only / Reporting user** — (Professional and Enterprise) view access to reporting without conversation or configuration access

### 3.2 Custom Roles (Professional and Enterprise)

Administrators can create custom roles under **Admin Dashboard > Roles & Permissions > Custom Roles**, combining specific permissions (Section 4) outside the standard role definitions. This is useful for scenarios such as a quality assurance reviewer who needs read access to conversations but no reply or configuration ability.

### 3.3 Assigning Roles

Roles are assigned per user, either individually or via bulk invitation, as described in Section 2. A user may hold only one role at a time within CloudDesk Chat, though custom roles can be configured to closely match a blended set of responsibilities where needed.
...
Permissions in CloudDesk Chat fall into several categories, each independently configurable for custom roles on Professional and Enterprise plans:

- **Conversation permissions** — view, reply, transfer, resolve, delete
- **Configuration permissions** — widget settings, routing rules, canned response management
...
Team Lead and Administrator roles inherit all Agent-level conversation permissions by default; this inheritance is not configurable for standard roles but can be adjusted when building a custom role from scratch.
...
CloudDesk Chat encrypts data in transit (TLS 1.2+) and at rest (AES-256) by default, consistent with the Corvex Cloud platform-wide standard. These settings are managed by Corvex and are not independently configurable, ensuring a consistent security baseline across all accounts.
...
CloudDesk Chat maintains an audit log of security- and configuration-relevant actions, including:

- User invitations, role changes, deactivations, and removals
- Widget and routing configuration changes
...
Corvex recommends periodic review of audit logs, particularly after a role change, an offboarding, or a suspected security incident, to confirm that account activity matches expectations.

---

## 7. Backup

### 7.1 How Backup Works in CloudDesk Chat

As a cloud-hosted service, CloudDesk Chat data is not backed up using traditional local or on-premises backup tools. Instead, Corvex maintains infrastructure-level backups of all customer data as part of the underlying platform's operational reliability practices, covering conversation history, customer records, and configuration data.
...
2. Select the data scope: conversation history, customer records, or full account configuration.
...
Standard exports are provided in CSV format for tabular data (e.g., conversation summaries, customer lists) and JSON format for structured configuration data (e.g., routing rules, widget settings).
...
### 8.2 Restoring Individual Conversations or Records

For more routine situations — such as a conversation accidentally deleted by an agent — administrators can restore individual items under **Admin Dashboard > Data > Recently Deleted**, provided the deletion occurred within the account's recovery window (30 days on Starter and Professional; extended windows available on Enterprise).

1. Navigate to **Admin Dashboard > Data > Recently Deleted**.
2. Locate the deleted conversation or record using search or filters.
3. Click **Restore**. The item is returned to its prior state, including associated tags and notes.

### 8.3 Restoring from a Prior Export
...
Navigate to **Admin Dashboard > Integrations** to view, add, or remove integrations connected to your CloudDesk Chat account.

### 9.2 Native Integrations

Available native integrations (CRM, e-commerce, collaboration tools) are listed with a **Connect** button. Selecting one walks you through an authentication flow specific to that integration, consistent with the integration categories described in the CloudDesk Chat Product Overview. Native integrations beyond a limited starter set require a Professional or Enterprise plan.
...
Full management of API keys and webhooks is handled through the CloudDesk API Platform Developer Portal, linked from **Admin Dashboard > Integrations > Developer Portal**, rather than duplicated within the CloudDesk Chat Admin Dashboard itself.
...
The Integrations section displays a status indicator for each connected integration (Healthy, Warning, Error), allowing administrators to quickly identify a failing sync without needing to investigate individual conversations first.
...
The Admin Dashboard's account health summary (Section 1.3) provides a real-time view of seat usage, storage usage, and outstanding security recommendations.
...
### 10.5 Integration and API Monitoring

Integration health (Section 9.6) and API usage (available via the CloudDesk API Platform Developer Portal) together give administrators visibility into the technical health of all connected systems, not just the core chat product.
...
### 11.3 Canned Response and Tag Hygiene

Over time, canned response libraries and tag taxonomies can accumulate unused or duplicate entries. Corvex recommends a periodic review (quarterly or semi-annually, depending on team size) to archive unused canned responses and consolidate overlapping tags, keeping reporting in CloudDesk Analytics meaningful.

### 11.4 Seat and Plan Maintenance
...
| Export conversation data | Admin Dashboard > Data > Export |
| Restore an accidentally deleted conversation | Admin Dashboard > Data > Recently Deleted |
| Connect a CRM integration | Admin Dashboard > Integrations > [CRM Name] > Connect |
...
| Update widget appearance or routing | Admin Dashboard > Widget & Channels |
| View or revoke active user sessions | Admin Dashboard > Security > Active Sessions |

---

*This Administrator Guide covers standard administrative functionality for CloudDesk Chat. For information on plan-specific feature availability, refer to the CloudDesk Chat Product Overview and the Corvex Cloud Pricing Guide. For end-user instructions, refer to the CloudDesk Chat User Manual.*
