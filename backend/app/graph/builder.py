...
from app.graph.collaboration import record_agent_step, route_after_confidence, route_supervisor
from app.core.confidence import confidence_node
...
def build_graph(intent_classifier: IntentClassifierNode | None = None, agents: dict[str, object] | None = None, memory_manager: MemoryManager | None = None, supervisor: SupervisorAgent | None = None):
    ...
    for name in configured:
        if name != "escalation_agent":
            graph.add_edge(name, "record_agent_step")
    graph.add_edge("record_agent_step", "confidence_check")
    if "escalation_agent" in configured:
        graph.add_conditional_edges("confidence_check", route_after_confidence, {"escalation_agent": "escalation_agent", "supervisor": "supervisor"})
        graph.add_edge("escalation_agent", "save_memory")
    else:
        graph.add_edge("confidence_check", "supervisor")
